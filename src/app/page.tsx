...
export default async function VehiclePage(props: any) {
  ...
  // ✅ Traer todas las imágenes del vehículo
  const extraImages = await db
    .select()
    .from(vehicleImages)
    .where(eq(vehicleImages.vehicleId, id));

  // ✅ Combinar imagen principal + extras sin duplicados
  const allImages = [
    vehicle.imageUrl,
    ...extraImages
      .map((img) => img.imageUrl)
      .filter((url) => url !== vehicle.imageUrl),
  ];
      ...
      <div className="mx-auto max-w-7xl px-6 py-20">
        ...
        <div className="mb-14">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-6">
            ...
            <span className={`rounded-full px-5 py-1.5 text-sm font-bold capitalize ${statusStyles[vehicle.status]}`}>
              {vehicle.status}
            </span>
          </div>
        </div>
        ...
        <div className="grid gap-12 lg:grid-cols-[1.7fr_1fr] items-start">
          ...
          {/* CARRUSEL */}
          <ImageCarousel images={allImages} name={vehicle.name} />
